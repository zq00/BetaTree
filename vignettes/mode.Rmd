...
```{r}
modes <- find_mode(hist = hist, d = d, cutoff = 1000)
```

The `find_mode` function returns a list with three components: `mode` returns the index of the modes, `hist` returns the input histogram, and `g` returns the adjacency graph of regions in the histogram. 
...
In the next section, we describe details of the `find_mode` function. 
