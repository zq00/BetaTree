...
`nd` contains the number of *bounded* regions at each depth of the tree (the root node has depth = 0). This is useful when we adjust the confidence levels at each depth to account for multiple CI. Here, at depth 5 there are `r tree$nd[5]` bounded regions. 
...
This confidence interval is valid for only the region $R_k$. If we want to simultaneously cover all the regions  at level $(1-\alpha)$, we can redefine the confidence level for each region such that the total probability of mis-coverage is at most $\alpha$. For example, we can use a **weighted bonferroni** adjustment, which assigns the same level for every region in the same tree depth. The confidence level for regions at tree depth $D$ is defined as $1-\alpha_D$, <span style="color: red;">The following formula needs to be re-written after checking with Guenther</span>

$$
\hat{\alpha}_D = \frac{\alpha}{N_D (D_{\max} - D + 2)   \sum_{B=2}^{D_{\max}-2}\frac{1}{B}}.
$$
