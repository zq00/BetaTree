...
Here, $D_{\max}$ is the maximum depth of the k-d tree, $D_{\min}$ is the smallest depth at which there are bounded regions, $N_D$ is the number of *bounded* regions at depth $D$, and the factor $\sum_{B=2}^{D_{\max}-D_{\min} + 2}\frac{1}{B}$ ensures that $\hat{\alpha}_D$ adds up to $\alpha$. 
